    ...
    def _read_stdout(self) -> None:
        ...
        while "\n" in self._stdout:
            ...
            if event == "log":
                ...
                step = str(message.get("step") or "")
                if session_id == "live-mosaic":
                    try:
                        pane = int(message.get("pane") or 0)
                    except (TypeError, ValueError):
                        pane = 0
                    try:
                        ...
                        total = 0
                    self.mosaicProgressReceived.emit(pane, total, step)
                else:
...
# Hold the last RTSP frame in a mosaic pane after stack is commanded. Tracking
# needs a few seconds to settle, and the next HTTP stacked.jpg can take longer.
_MOSAIC_TRACK_SETTLE_S = 3.0
_MOSAIC_LAST_FRAME_WAIT_S = 5.0
...
_ASTRO_SHOOTING_MODES = {2, 8, 9, 10}


def _shooting_mode_int(value: Any) -> int:
...
def preview_needs_rtsp_restart(telemetry: dict[str, Any] | None) -> bool:
    ...
    After a stack the Dwarf 3 often leaves ``stream_type=JPEG`` with RTSP
    ports open but producing no frames. ``enter_camera`` reports success and
    live view still stays dark. Photo mode starts both RTSP encoders.
    """
    if _preview_capture_running(telemetry):
        return False
    snap = telemetry or {}
    return (
        _stream_type_name(snap.get("stream_type")) == "JPEG"
        or _stream_type_name(snap.get("stream_type_wide")) == "JPEG"
    )


def preview_should_restore_dso(
    telemetry: dict[str, Any] | None,
    persisted_mode: int = 0,
) -> bool:
    """True when leftover-JPEG recovery must return to DSO after photo mode.

    Photo mode is what restarts RTSP. If the scope was already in DSO, put
    that shooting mode back once the encoders are live.
    """
    if not preview_needs_rtsp_restart(telemetry):
        return False
    mode = _shooting_mode_int((telemetry or {}).get("shooting_mode"))
    if mode == 2:
        return True
    return persisted_mode == 2 and mode != 1


def preview_should_preserve_shooting_mode(
    telemetry: dict[str, Any] | None,
    persisted_mode: int = 0,
) -> bool:
...
def preview_should_skip_go_live(
    ...
    Leftover stacking JPEG still needs GoLive plus photo mode to bring RTSP
    back.
    """
    snap = telemetry or {}
...
def preview_can_attach_rtsp(
    ...
    *,
    ...
    if preview_needs_rtsp_restart(telemetry):
        return False
    if tele_playing or wide_playing:
        return True
...
def mosaic_result_should_drop_sheet(
    *,
    preview_result: bool,
    held: bool = False,
    frames_active: bool = False,
    columns: int = 1,
    rows: int = 1,
    mosaic_running: bool = False,
) -> bool:
    """True when Dismiss or Start live view must clear contact-sheet and sky stills."""
    if mosaic_running:
        return False
    if held:
        return True
    ...
    try:
        ...
        mosaic = False
    return bool(frames_active and mosaic)
...
def mosaic_result_pane(current_index: int, phase: str) -> int:
    ...
    frame there duplicates pane N into pane N+1. A failed GOTO has not stacked
    that pane, so freeze the previous cell or nothing.
    """
    try:
...
def mosaic_overlay_live_pane(
    *,
    ...
    RTSP may fill the pane during GOTO/settle and after stack is commanded
    until HTTP stacking preview takes over. Leftover stacked.jpg must not
    paint the next cell.
    """
    if may_copy:
    ...
    if stacking_preview or not preview_playing:
        return 0
    slew = mosaic_slew_live_pane(current_index, phase, active)
    if slew >= 1:
        return slew
    if str(phase or "").strip().lower() == "stacking":
        try:
            index = int(current_index or 0)
        except (TypeError, ValueError):
            return 0
...
def mosaic_hold_pane(current_index: int, phase: str, active: bool = True) -> int:
    """Pane that may keep the last live camera frame until stacking JPEG starts."""
    stacking = mosaic_live_pane(current_index, phase, active)
    if stacking >= 1:
        return stacking
    slew = mosaic_slew_live_pane(current_index, phase, active)
    if slew >= 1:
    ...
    try:
        ...
        return 0
    if not active or index < 1:
        return 0
    if str(phase or "").strip().lower() in {"goto", "stacking"}:
        return index
    return 0


def mosaic_should_refresh_slew_placeholder(
    *,
    stacking_preview: bool,
    frozen: bool,
    may_copy: bool,
    has_live_frame: bool,
    tracking: bool = False,
    ...
    """Incoming RTSP frames may update the last-frame placeholder.

    Mid-slew frames stay off the contact sheet until tracking has settled
    or stack has already been commanded.
    """
    if not has_live_frame or stacking_preview or frozen or may_copy:
        return False
    if stacking:
        return True
    try:
        elapsed = float(track_elapsed_s or 0)
        settle = max(0.0, float(settle_s or 0))
    except (TypeError, ValueError):
        return False
    return bool(tracking and elapsed >= settle)


def mosaic_slew_frame_ready(
    *,
    has_frame: bool,
    ...
    """True when the last live frame can be held and stacking preview may switch."""
    if timed_out:
        return True
    if not has_frame:
        return False
...
def mosaic_should_hold_last_live_frame(pane: int) -> bool:
    """Defer HTTP stacking preview only for a live mosaic cell.

    A single STACK has no contact-sheet pane. Holding then waits the
    timeout with no frame, and used to copy every RTSP frame on the GUI
    thread until that timeout.
    """
    try:
        return int(pane or 0) >= 1
    except (TypeError, ValueError):
        return False
...
def mosaic_capture_continues(
    *,
    ...
    if str(live_phase or "").strip() and worker_running:
        return True
    try:
        panes = int(firmware_panes or 1)
        pane = int(firmware_pane or 1)
    except (TypeError, ValueError):
        return False
    return bool(session_running and panes > 1 and pane < panes)


def mosaic_progress_phase(step: str) -> str:
    """Classify a live-mosaic progress line so leftover frames stay off the next pane."""
...
def mosaic_live_still_camera() -> str:
    """Camera copied into a finished mosaic cell.

    After a pane stack, GoLive closes tele and wide JPEG comes back. Freezing
    that wide frame is what put trees into pane 3 of a tele mosaic.
    """
    return "tele"


def mosaic_group_sessions(sessions, device_id: str, group_id: str):
    """Sessions in one mosaic group. JsonRepository lists via all(), not values()."""
    if not group_id:
        return []
    return [
        item
...
def mosaic_preview_should_open_wide(*, mosaic_running: bool) -> bool:
    """PIP wide-open is fine in live view, not while a mosaic still owns the scope."""
    return not mosaic_running


def mosaic_stack_preview_should_retarget(
    live_pane: int,
    stream_pane: int,
    stacking: bool,
    mode_changed: bool,
    ...
    """Reopen the stacking JPEG when a later mosaic pane starts on the same URL."""
    if mode_changed or urls_changed:
        return True
    if not stacking:
        return False
    try:
        pane = int(live_pane or 0)
        previous = int(stream_pane or 0)
    except (TypeError, ValueError):
        return False
...
def mosaic_should_copy_live_still(
    *,
    camera: str,
    phase: str = "",
    pane: int = 0,
    ...
    stacked: int = 0,
    taken: int = 0,
    seen_reset: bool = False,
    stack_preview: bool = True,
) -> bool:
    """True only for a tele frame that belongs to the pane currently stacking.

    After a pane stack, GoLive closes tele and wide JPEG comes back on the
    same HTTP URL. Freezing that leftover is the trees-in-pane-3 bug. Slew
    and settling frames must not land in pane N+1 either.
    """
    if str(camera or "").strip().lower() != mosaic_live_still_camera():
        return False
    phase_name = str(phase or "").strip().lower()
    if phase_name and phase_name != "stacking":
        return False
    if not capturing or not stack_preview:
        return False
    return mosaic_accept_live_frame(
        pane=pane,
        stream_pane=stream_pane,
        stacked=stacked,
        taken=taken,
...
def mosaic_should_publish_pane_url(
    *,
    pane: int,
    has_still: bool = False,
    goto_failed: bool = False,
    failed_pane: int = 0,
) -> bool:
    """SKY overlay URLs are only for panes that actually stacked a tele still.

    A failed GOTO must not publish leftover live video into that cell.
    Previously stacked panes keep their URLs.
    """
    try:
        index = int(pane or 0)
    except (TypeError, ValueError):
        return False
    if index < 1 or not has_still:
        return False
    if goto_failed:
        try:
            failed = int(failed_pane or 0)
        except (TypeError, ValueError):
            failed = 0
        if failed < 1 or index == failed:
            return False
    return True


def mosaic_goto_failed_result(result: Any) -> bool:
    """True when mosaic ended because a pane GOTO did not acquire the field."""
    text = str(result or "").lower()
    if not text:
        return False
    if "not stacking this pane" in text:
        return True
    return "goto" in text and "fail" in text
...
class AppBackend(QObject):
    ...
    def __init__(self, data_root: Path, parent: QObject | None = None):
        ...
        self._manual_capture_started: dict[str, str] = {}
        self._manual_capture_peak: dict[str, int] = {}
        ...
        self._resume_mosaic_attempted: set[str] = set()
        ...
        self._sky_named_coords: dict[str, dict[str, Any]] = {}
        self._sky_mosaic_center: Target | None = None
        self._catalog_resolve_key = ""
        ...
        self._mosaic_hold_urls: tuple[str, str] | None = None
    ...
    def _create_worker(self, device: Device, reconnect: bool = False) -> TelescopeProcess:
        ...
        worker.mosaicProgressReceived.connect(
    ...
    def _on_telemetry(self, device_id: str, data: dict[str, Any]) -> None:
        ...
        self._track_manual_capture(device_id, previous, current)
        ...
        if (
            previous.get("tracking_target") != current.get("tracking_target")
            or previous.get("capture_target") != current.get("capture_target")
        ...
        if device_id == self._selected_device_id and (
            previous.get("tracking_state") != current.get("tracking_state")
            or previous.get("tracking_target") != current.get("tracking_target")
            or previous.get("capture_target") != current.get("capture_target")
            or previous.get("shooting_mode") != current.get("shooting_mode")
        ):
            self.skyTargetChanged.emit()
    ...
    def _photo_capture_clock_active(self) -> bool:
        for raw in self._device_telemetry.values():
    ...
    def _decorate_history_groups(self, items: list[dict[str, Any]]) -> list[dict[str, Any]]:
        counts: dict[str, int] = {}
        for item in items:
        ...
        for item in items:
            key = str(item.get("group_key") or "")
            pane_count = counts.get(key, 1)
            grouped = pane_count > 1
            item["pane_count"] = pane_count
            item["is_grouped"] = grouped
            if not grouped:
                item["group_key"] = f"session:{item.get('id', '')}"
        return items
    ...
    def history(self) -> list[dict[str, Any]]:
        ...
        for record in sorted(self.store.history.all(), key=lambda item: item.recorded_at, reverse=True):
            ...
            group_id = str(getattr(record, "mosaic_group_id", "") or "").strip()
            if not group_id and session is not None:
                group_id = str(session.mosaic.group_id or "").strip()
            title = mosaic_group_title(record.target_name, group_id)
            pane_like = is_mosaic_pane_name(record.target_name) or int(record.mosaic_panes or 1) > 1
            if not group_id and pane_like:
                group_id = f"{title}|{record.device_id}|{data['date']}"
            data["group_id"] = group_id
            data["group_title"] = title
            data["pane_name"] = record.target_name
            data["pane_index"] = pane_sort_key(record.target_name)[0]
            grid = ""
            if session is not None:
                mosaic = session.mosaic
                grid = mosaic.grid_text
            ...
            if group_id and (pane_like or (session is not None and session.mosaic.group_id)):
                ...
                data["group_key"] = f"session:{record.id}"
    ...
    def _mosaic_pa_state(self, target: Target | None = None) -> MosaicPa:
        device = self._schedule_device()
        if device is None:
            return MosaicPa(0.0, MOSAIC_PA_DEFAULT, False, "")
        sky = target
        ...
        return resolve_device_mosaic_pa(
            ...
            ra_hours=None if sky is None else sky.ra_hours,
            dec_degrees=None if sky is None else sky.dec_degrees,
            mount_mode=telemetry.get("mount_mode"),
        )

    def _mosaic_pa(self, target: Target | None = None) -> float:
        return self._mosaic_pa_state(target).degrees

    def _overlay_pa_target(self, harvested: Target | None) -> Target | None:
        locked = self._sky_target
        if locked is not None and locked.ra_hours is not None and locked.dec_degrees is not None:
            return locked
        if harvested is not None and harvested.ra_hours is not None and harvested.dec_degrees is not None:
            return harvested
        return None

    def _resolved_mosaic_pa_state(self, target: Target | None, position_angle: Any = None) -> MosaicPa:
        """PA for overlay/STACK/save. Ignore a QML spinbox value when mosaic_pa is unset."""
        device = self._schedule_device()
        if device is not None:
            return self._mosaic_pa_state(target)
        try:
            pa = float(position_angle) % 360.0
        except (TypeError, ValueError):
            return MosaicPa(0.0, MOSAIC_PA_DEFAULT, False, "")
        return MosaicPa(pa, MOSAIC_PA_DEFAULT, False, "")

    def _resolved_mosaic_pa(self, target: Target | None, position_angle: Any = None) -> float:
        return self._resolved_mosaic_pa_state(target, position_angle).degrees

    def _log_overlay_mosaic_pa(self, resolved: MosaicPa) -> None:
        key = (
            round(float(resolved.degrees), 1),
            bool(resolved.south_up),
            str(resolved.mount_mode or ""),
            str(resolved.source or ""),
        )
        if key == self._logged_overlay_pa:
            return
        self._logged_overlay_pa = key
        logging.getLogger(__name__).info(
            "mosaic overlay PA %.1f south_up=%s mount_mode=%s source=%s",
            resolved.degrees,
            "true" if resolved.south_up else "false",
            resolved.mount_mode or "none",
            resolved.source,
        )

    @Property(float, notify=mosaicPaChanged)
    def mosaicPa(self) -> float:
        return self._mosaic_pa()

    @Property(str, notify=mosaicPaChanged)
    def mosaicPaSource(self) -> str:
        return self._mosaic_pa_state().source

    @Property(str, notify=mosaicPaChanged)
    def mosaicPaChip(self) -> str:
    ...
    def _clamp_mosaic_axis(self, value: Any, fallback: int = 1) -> int:
        try:
            axis = int(value)
        except (TypeError, ValueError):
            axis = fallback
        return max(1, min(MAX_MOSAIC_AXIS, axis))

    def _clamp_mosaic_overlap(self, value: Any, fallback: float = 0.2) -> float:
        try:
            overlap = float(value)
        except (TypeError, ValueError):
            overlap = fallback
    ...
    def _load_sky_mosaic_grid(self) -> None:
        store = QSettings()
    ...
    def _sky_mosaic_grid(self) -> tuple[int, int, float]:
        return (
            ...
            self._clamp_mosaic_overlap(self._sky_mosaic_overlap, 0.2),
        )

    @Property(int, notify=skyMosaicGridChanged)
    def mosaicColumns(self) -> int:
        return self._sky_mosaic_grid()[0]

    @Property(int, notify=skyMosaicGridChanged)
    def mosaicRows(self) -> int:
        return self._sky_mosaic_grid()[1]
    ...
    @Slot(int, int, float)
    def setSkyMosaicGrid(self, columns: int, rows: int, overlap: float) -> None:
        next_columns = self._clamp_mosaic_axis(columns, 1)
        next_rows = self._clamp_mosaic_axis(rows, 1)
        next_overlap = self._clamp_mosaic_overlap(overlap, 0.2)
        if (
            next_columns == self._sky_mosaic_columns
            and next_rows == self._sky_mosaic_rows
            and abs(next_overlap - self._sky_mosaic_overlap) < 1e-6
        ):
            return
    ...
    def _live_mosaic_center(self) -> Target | None:
        sky = self._sky_target
        name = str(getattr(sky, "name", None) or "").strip()
        coords = self._target_coords(sky)
        locked = (
            Target(name=name, ra_hours=coords[0], dec_degrees=coords[1])
            if coords is not None
            else None
        )
        tracked = self.trackedSkyTarget
        tracked_name = str(tracked.get("name") or "").strip()
        tracked_coords = self._target_coords(tracked)
        tracked_target = (
            Target(
                name=tracked_name or "Tracked target",
                ra_hours=tracked_coords[0],
                dec_degrees=tracked_coords[1],
            )
            if tracked_name and tracked_coords is not None
            else None
        )
        named = named_mosaic_center(locked, tracked_target)
        if named is not None:
            return named
        parent = mosaic_group_title(tracked_name or name)
        parent_coords = self._cached_catalog_coords(parent) if parent else None
        if parent_coords is None and parent:
            parent_coords = self._commanded_sky_coords(parent)
        parent_target = (
            Target(name=parent, ra_hours=parent_coords[0], dec_degrees=parent_coords[1])
            if parent and parent_coords is not None
            else None
        )
        named = named_mosaic_center(parent_target)
        if named is not None:
            return named
        center = self._sky_mosaic_center
        overlay_coords = self._target_coords(center)
        if overlay_coords is not None:
            return Target(
                name=str(getattr(center, "name", None) or "Sky target"),
                ra_hours=overlay_coords[0],
                dec_degrees=overlay_coords[1],
            )
        return self._sky_view_center()

    def _sky_view_center(self) -> Target | None:
        store = QSettings()
        store.beginGroup("sky")
        saved = store.value("viewSaved", False)
        ra = store.value("viewRaHours")
        dec = store.value("viewDecDegrees")
        store.endGroup()
        flagged = saved in {True, 1, "1"} or str(saved).strip().lower() in {"true", "yes"}
        coords = self._target_coords({"ra_hours": ra, "dec_degrees": dec})
        if coords is None or not flagged:
            return None
        return Target(name="FOV centre", ra_hours=coords[0], dec_degrees=coords[1])

    def _preview_pane_payload(self, panes: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    def _cached_sky_preview_panes(
        ...
        if len(panes) < 2:
            return []
        points = {
            (round(float(item["ra_hours"]), 5), round(float(item["dec_degrees"]), 5))
            for item in panes
        }
        if len(points) < 2:
            return []
    ...
    def _live_mosaic_for(self, device_id: str = "") -> dict[str, Any] | None:
        owner = str(device_id or self._selected_device_id or "")
        return self._live_mosaic.get(owner)

    def _live_mosaic_panes(self, live: dict[str, Any]) -> list[dict[str, Any]]:
        panes = live.get("panes")
        if isinstance(panes, list) and panes:
            return [dict(item) for item in panes if isinstance(item, dict)]
        payload: list[dict[str, Any]] = []
        for index, member in enumerate(list(live.get("members") or []), start=1):
            if not isinstance(member, Session):
                continue
            ...
            payload.append({
                ...
                "name": member.name,
                "row": int(member.mosaic.row or 0),
                "column": int(member.mosaic.column or 0),
                "session_id": member.id,
            })
        return payload

    def _hydrate_live_mosaic(self, device_id: str, payload: dict[str, Any]) -> dict[str, Any] | None:
        ...
        try:
            ...
            current_index = max(1, int(payload.get("current_index") or 1))
        except (TypeError, ValueError):
            return None
        group_id = str(payload.get("group") or f"live-mosaic-{device_id[:8]}")
        stored_by_id = {
            item.id: item
        ...
        stored_by_index = {
            mosaic_pane_index(item.name): item
            for item in stored_by_id.values()
            if mosaic_pane_index(item.name)
        }
        ...
        for pane in panes:
            ...
            if ra != ra or dec != dec:
                continue
            name = str(pane.get("name") or f"Pane {index}").strip() or f"Pane {index}"
            try:
                row = int(pane.get("row") or 0)
                column = int(pane.get("column") or 0)
            except (TypeError, ValueError):
            ...
            stored = stored_by_id.get(session_id) or stored_by_index.get(index)
            if stored is not None:
                ...
                members.append(
                    Session(
                        name=name,
                        target=Target(
                            name=mosaic_group_title(name, group_id),
                        ...
                        status=SessionStatus.RUNNING if index == current_index else SessionStatus.PLANNED,
                        current_step=str(payload.get("phase") or "Recovered after restart"),
                    )
                )
            cleaned.append({
                "index": index,
                ...
                "row": row,
                "column": column,
                "session_id": stored.id if stored is not None else members[-1].id,
            })
            self._remember_named_sky_coords(name, ra, dec)
        if len(members) < 2:
            return None
        current = members[min(current_index, len(members)) - 1]
        return {
            "group": group_id,
            "columns": columns,
            "rows": rows,
            "total": max(int(payload.get("total") or 0), len(members)),
            "current_index": min(current_index, len(members)),
            "phase": str(payload.get("phase") or "recovered") or "recovered",
            "label": str(payload.get("label") or current.name),
            "camera": str(payload.get("camera") or "tele"),
            "members": members,
            "current": current,
            "panes": cleaned,
            "stopping": False,
    ...
    def _live_mosaic_persist_payload(self, device_id: str, live: dict[str, Any]) -> dict[str, Any]:
        return {
            "device_id": device_id,
            ...
            "current_index": live.get("current_index"),
            "phase": live.get("phase"),
            "label": live.get("label"),
            "camera": live.get("camera") or "tele",
            "panes": self._live_mosaic_panes(live),
        }

    def _persist_live_mosaic(self, device_id: str) -> None:
        live = self._live_mosaic.get(device_id)
    ...
    def _restore_live_mosaics(self) -> None:
        recovered = 0
        for device_id, payload in self.store.load_live_mosaics().items():
            live = self._hydrate_live_mosaic(device_id, payload)
            if not live:
                self.store.clear_live_mosaic(device_id)
                continue
            self._live_mosaic[device_id] = live
            recovered += 1
            if device_id == self._selected_device_id:
                self.mosaic_frames.set_layout(
                    int(live.get("columns") or 1),
                    int(live.get("rows") or 1),
                    int(live.get("current_index") or 1),
                    True,
    ...
    def _maybe_resume_interrupted_mosaic(self, device_id: str, *, allow_idle: bool = False) -> None:
        if device_id in self._resume_mosaic_attempted:
            return
        session_id = self._active_sessions.get(device_id)
        if session_id and session_id not in self._command_panel_sessions:
            return
        live = self._live_mosaic.get(device_id)
        ...
        if plan is None:
            ...
            live["phase"] = ""
            self.store.clear_live_mosaic(device_id)
            self.add_log("notice", "Interrupted mosaic already finished on the telescope", device_id)
            self.mosaicPreviewChanged.emit()
            self._notify_devices()
            return
        start_index, join_current = plan
        self._resume_mosaic_attempted.add(device_id)
        self._resume_live_mosaic(device_id, start_index, join_current)

    def _resume_live_mosaic(self, device_id: str, start_index: int, join_current: bool) -> None:
        worker = self._workers.get(device_id)
        live = self._live_mosaic.get(device_id)
        if not worker or not live:
            return
        panes = self._live_mosaic_panes(live)
        if len(panes) < 2:
            self.store.clear_live_mosaic(device_id)
            return
        live["panes"] = panes
        live["worker_running"] = True
        live["phase"] = "stacking" if join_current else "goto"
        live["current_index"] = start_index
        if 1 <= start_index <= len(list(live.get("members") or [])):
            members = list(live["members"])
            current = replace(members[start_index - 1], status=SessionStatus.RUNNING, current_step="Joining mosaic")
            members[start_index - 1] = current
            live["members"] = members
            live["current"] = current
            live["label"] = current.name
            if self.store.sessions.get(current.id):
                self._adopt_command_session(device_id, current, step="Joining mosaic")
        worker.busy = True
        worker.availabilityChanged.emit()
        self.mosaic_frames.set_layout(
            int(live.get("columns") or 1),
        ...
        grid = f"{int(live.get('columns') or 1)}×{int(live.get('rows') or 1)}"
        self.add_log("notice", f"Mosaic resumed · {grid} from pane {start_index}", device_id)
        self._toast(
            "Mosaic resumed",
            "info",
            f"Joining pane {start_index} of {int(live.get('total') or start_index)}",
        )

        def done(ok: bool, result: Any) -> None:
            self._finish_live_mosaic(device_id, ok, result)
    ...
    def _target_coords(self, source: Any) -> tuple[float, float] | None:
        if source is None:
            return None
        if isinstance(source, dict):
            ra = source.get("ra_hours")
            dec = source.get("dec_degrees")
        else:
            ra = getattr(source, "ra_hours", None)
            dec = getattr(source, "dec_degrees", None)
        try:
            ra_h = float(ra)
            dec_d = float(dec)
        except (TypeError, ValueError):
    ...
    def _remember_track_target(self, result: Any, fallback_name: str = "") -> None:
        ...
        coords = self._target_coords(data)
        name = str(data.get("name") or fallback_name or "").strip()
        if coords is None and self._sky_target is not None:
            coords = self._target_coords(self._sky_target)
            name = name or str(self._sky_target.name or "")
        if coords is None:
            session = self._current_session_view or {}
            coords = self._target_coords(session.get("target"))
            name = name or str(session.get("target_name") or "")
        if coords is None:
            return
        self._remember_named_sky_coords(name, coords[0], coords[1])
        if is_mosaic_pane_name(name):
            self._set_preview_coords(name, coords)
            return
        current = self._named_sky_coords(name)
        if current is not None:
            self._set_sky_target(Target(name=name or "Tracked target", ra_hours=current[0], dec_degrees=current[1]))
            return
        self._set_sky_target(Target(name=name or "Tracked target", ra_hours=coords[0], dec_degrees=coords[1]))

    def _remember_named_sky_coords(self, name: str, ra_hours: Any, dec_degrees: Any) -> None:
        label = str(name or "").strip()
        coords = self._target_coords({"ra_hours": ra_hours, "dec_degrees": dec_degrees})
        if not label or coords is None:
            return
        self._sky_named_coords[label.lower()] = {
            "name": label,
            "ra_hours": coords[0],
    ...
    def _commanded_sky_coords(self, name: str) -> tuple[float, float] | None:
        text = str(name or "").strip()
        if not text:
            return None
        coords = self._target_coords(self._sky_named_coords.get(text.lower()))
        if coords is not None:
            return coords
        wanted = self._normalize_target_name(text)
        if wanted:
            for key, item in self._sky_named_coords.items():
                if self._normalize_target_name(key) == wanted:
                    coords = self._target_coords(item)
                    if coords is not None:
                        return coords
        live = self._live_mosaic_for()
        if not live:
            return None
        index = mosaic_pane_index(text) if is_mosaic_pane_name(text) else None
        parent = mosaic_group_title(text)
        for pane in self._live_mosaic_panes(live):
            pane_name = str(pane.get("name") or "")
            if wanted and self._normalize_target_name(pane_name) == wanted:
                coords = self._target_coords(pane)
                if coords is not None:
                    return coords
            if index is not None and int(pane.get("index") or 0) == index:
                pane_parent = mosaic_group_title(pane_name)
                if not parent or not pane_parent or parent.casefold() == pane_parent.casefold():
                    coords = self._target_coords(pane)
                    if coords is not None:
                        return coords
        return None

    def _normalize_target_name(self, name: str) -> str:
        text = str(name or "").lower().replace("γ", "gamma").replace("*", " ")
        text = re.sub(r"\b(name|catalogue|catalog)\b", " ", text)
        return re.sub(r"[^a-z0-9]+", "", text)

    def _target_names_match(self, left: str, right: str) -> bool:
        first = self._normalize_target_name(left)
        second = self._normalize_target_name(right)
        if not first or not second:
            return False
        if first == second:
            return True
        if is_mosaic_pane_name(left) or is_mosaic_pane_name(right):
            return False
        shorter, longer = (first, second) if len(first) <= len(second) else (second, first)
        return len(shorter) >= 8 and shorter in longer

    def _is_catalog_hit(self, hit: Any) -> bool:
        return isinstance(hit, dict) and str(hit.get("source") or "") == "catalog" and self._target_coords(hit) is not None

    def _catalog_hit_for(self, name: str) -> dict[str, Any] | None:
        text = str(name or "").strip()
        if not text:
            return None
        hit = self._sky_catalog_cache.get(text.lower())
        if self._is_catalog_hit(hit):
            return hit
        if is_mosaic_pane_name(text):
            return None
        wanted = self._normalize_target_name(text)
        for key, item in self._sky_catalog_cache.items():
            if not self._is_catalog_hit(item):
                continue
            if self._normalize_target_name(key) == wanted or self._target_names_match(key, text):
                return item
            aliases = item.get("aliases") or []
            if any(self._target_names_match(str(alias), text) for alias in aliases):
                return item
        return None

    def _cached_catalog_coords(self, name: str) -> tuple[float, float] | None:
        return self._target_coords(self._catalog_hit_for(name))

    def _named_sky_coords(self, name: str) -> tuple[float, float] | None:
        return self._commanded_sky_coords(name) or self._cached_catalog_coords(name)

    @Property("QVariantMap", notify=skyTargetChanged)
    def trackedSkyTarget(self) -> dict[str, Any]:
        device_id = str(self._selected_device_id or "")
        telemetry = self._device_telemetry.get(device_id) or {}
        tracking = telemetry.get("tracking_state") == "running"
        dso = telemetry.get("shooting_mode") == 2
        name = str(telemetry.get("capture_target") or telemetry.get("tracking_target") or "").strip()
        if not name and self._sky_target is not None:
            name = str(self._sky_target.name or "").strip()
        coords = self._named_sky_coords(name)
        return {
            "name": name,
            "ra_hours": coords[0] if coords else None,
            "dec_degrees": coords[1] if coords else None,
            "tracking": tracking,
            "dso": dso,
            "available": bool(name or coords),
        }

    def _apply_catalog_hit(
        self, device_id: str, hit: dict[str, Any], *, announce: bool, query_name: str = ""
    ) -> None:
        coords = self._target_coords(hit)
        catalog_name = str(hit.get("name") or "").strip()
        name = str(query_name or catalog_name).strip()
        if coords is None or not name or is_mosaic_pane_name(name) or is_mosaic_pane_name(catalog_name):
            return
        stored = dict(hit)
        stored["name"] = name
        stored["source"] = "catalog"
        keys = [catalog_name.lower()]
        if not is_mosaic_pane_name(name):
            keys.append(name.lower())
        for alias in stored.get("aliases") or []:
            alias_text = str(alias).strip()
            if alias_text and not is_mosaic_pane_name(alias_text):
                keys.append(alias_text.lower())
        for key in keys:
            if key:
                self._sky_catalog_cache[key] = stored
        if device_id == self._selected_device_id:
            self._set_sky_target(Target(name=name, ra_hours=coords[0], dec_degrees=coords[1]))
            self._set_preview_coords(name, coords)
        if announce:
            self.add_log(
                "info",
                f"Catalog position for {name}: {self._format_sky_coords(coords)} "
                "(firmware reports the target name only, not RA/Dec)",
                device_id,
            )

    def _schedule_catalog_resolve(self, device_id: str, name: str) -> None:
        text = str(name or "").strip()
        if not text:
            return
        commanded = self._commanded_sky_coords(text)
        if commanded is not None:
            self._set_preview_coords(text, commanded)
            if self._catalog_resolve_key == f"{device_id}:{text.lower()}":
                self._catalog_resolve_key = ""
            return
        if is_mosaic_pane_name(text):
            return
        cached = self._catalog_hit_for(text)
        if cached:
            self._apply_catalog_hit(device_id, cached, announce=False, query_name=text)
            return
        key = f"{device_id}:{text.lower()}"
        if key == self._catalog_resolve_key:
            return
        retry_at = self._catalog_resolve_cooldown.get(key, 0.0)
        if retry_at and time.monotonic() < retry_at:
            return
        self._catalog_resolve_key = key

        def run() -> None:
            try:
                hit = resolve_sky_catalog_target(text)
                self._asyncResult.emit(
                    "skyCatalogResolve",
                    (True, {"device_id": device_id, "name": text, "hit": hit}),
                )
            except Exception as exc:
                self._asyncResult.emit(
                    "skyCatalogResolve",
                    (False, {"device_id": device_id, "name": text, "error": str(exc)}),
                )

        threading.Thread(target=run, daemon=True).start()

    def _format_sky_coords(self, coords: tuple[float, float] | None) -> str:
        if coords is None:
            return ""
        ra, dec = coords
    ...
    def _set_preview_coords(self, name: str, coords: tuple[float, float] | None) -> None:
        ...
        label = str(name or "").strip()
        if text == self._preview_coord_text and label == self._preview_coord_name:
            return
        self._preview_coord_text = text
        self._preview_coord_name = label
        self.previewSkyCoordsChanged.emit()
        self.skyTargetChanged.emit()

    def _clear_preview_coords(self) -> None:
        self._set_preview_coords("", None)

    def _refresh_tracked_catalog(self, device_id: str) -> None:
        telemetry = self._device_telemetry.get(device_id) or {}
        name = str(telemetry.get("capture_target") or telemetry.get("tracking_target") or "").strip()
        tracking = telemetry.get("tracking_state") == "running"
    ...
    def _selected_sky_coords(self) -> tuple[float, float] | None:
        if not self._preview_coord_text:
            return None
        return self._cached_catalog_coords(self._preview_coord_name)

    @Slot(str, float, float)
    def setSkyMapTarget(self, name: str, ra_hours: float, dec_degrees: float) -> None:
        coords = self._target_coords({"ra_hours": ra_hours, "dec_degrees": dec_degrees})
        label = str(name or "").strip() or "Sky map target"
        if coords is None:
            return
        current = str(
            (self._device_telemetry.get(str(self._selected_device_id or "")) or {}).get("tracking_target")
            or (self._device_telemetry.get(str(self._selected_device_id or "")) or {}).get("capture_target")
            or ""
        ).strip()
        if (
            not current
            or self._target_names_match(current, label)
            or sky_names_related(current, label)
        ):
            if is_mosaic_pane_name(label):
                self._remember_named_sky_coords(label, coords[0], coords[1])
                self._set_preview_coords(label, coords)
                return
            self._set_sky_target(Target(name=label, ra_hours=coords[0], dec_degrees=coords[1]))

    @Property(str, notify=previewSkyCoordsChanged)
    def previewSkyCoordText(self) -> str:
        return self._preview_coord_text

    @Property(bool, notify=previewSkyCoordsChanged)
    def previewSkyCoordVisible(self) -> bool:
        return bool(self._preview_coord_text)

    @Property("QVariantMap", notify=previewSkyCoordsChanged)
    def previewSkyCoords(self) -> dict[str, Any]:
        coords = self._selected_sky_coords()
        if coords is None or not self._preview_coord_text:
            return {"visible": False, "text": "", "ra_hours": None, "dec_degrees": None}
        return {
            "visible": True,
            "text": self._preview_coord_text,
            "ra_hours": coords[0],
            "dec_degrees": coords[1],
        }
    ...
    def _emit_sky_lock(self, name: str, coords: tuple[float, float], aliases: list[str] | None = None) -> None:
    ...
    def lockSkyToTrackedTarget(self) -> None:
        ...
        if not target.get("available"):
            self._toast("No tracked DSO target to show on the sky map", "warning")
            return
        if self._sky_lock_inflight:
            return
        name = str(target.get("name") or "").strip() or "Tracked target"
        commanded = self._commanded_sky_coords(name)
        cached = None if is_mosaic_pane_name(name) else self._catalog_hit_for(name)
        coords = commanded or self._target_coords(cached) or self._target_coords(target)
        if coords is not None and commanded is not None:
            self._emit_sky_lock(name, coords)
            return
        if coords is not None and self._is_catalog_hit(cached):
            self._emit_sky_lock(name, coords, list(cached.get("aliases") or []))
            return
        if is_mosaic_pane_name(name):
            self._toast(
                "Could not lock that target on the sky map",
                "warning",
                f"No stored pane coordinates for {name}",
            )
            return
        self._sky_lock_inflight = True

        def work() -> dict[str, Any]:
            resolved = resolve_sky_catalog_target(name)
            return resolved or {"need_pointing": True, "name": name}

        def run() -> None:
            try:
                self._asyncResult.emit("skyLockResolve", (True, work()))
            except Exception as exc:
                self._asyncResult.emit("skyLockResolve", (False, str(exc)))

        threading.Thread(target=run, daemon=True).start()

    @Slot(str, str)
    def reportSkyLockResult(self, status: str, name: str = "") -> None:
        ...
        if state == "missing":
            self._toast(
                "Could not lock that target on the sky map",
                "warning",
                f"Stellarium Web does not recognize {label}",
            )
        elif state == "error":
            self._toast("Could not lock that target on the sky map", "error")
    ...
    def _device_fov(
        ...
        if not camera_fov_plausible(fov_h, fov_v, choice, device.model if device is not None else None):
    ...
    def _mosaic_fov(self) -> tuple[float, float, str]:
        # Pane spacing and STACK MOSAIC stay on tele even when the live combo
        # is Wide. A running mosaic keeps mosaic_stack_camera so overlay boxes
        # match the ICRS points already commanded.
        device_id = str(self._selected_device_id or "")
        live = self._live_mosaic.get(device_id)
        camera = None
        if self._live_mosaic_running(device_id, live):
            camera = (live or {}).get("camera")
    ...
    def _mosaic_may_copy_live(self, pane: int, device_id: str = "") -> bool:
        ...
        return mosaic_should_copy_live_still(
            ...
            taken=taken,
            seen_reset=self._mosaic_seen_stack_reset,
            stack_preview=bool(self._preview_stack_mode),
        )
    ...
    def _mosaic_hold_pane(self, device_id: str = "") -> int:
        owner = str(device_id or self._selected_device_id or "")
        live = self._live_mosaic.get(owner) or {}
        try:
            index = int(live.get("current_index") or 0)
    ...
    def _mosaic_track_elapsed_s(self, device_id: str = "") -> tuple[bool, float]:
        owner = str(device_id or self._selected_device_id or "")
        telemetry = self._device_telemetry.get(owner) or {}
        tracking = telemetry.get("tracking_state") == "running"
        if not tracking:
            return False, 0.0
        if not self._mosaic_track_since:
            self._mosaic_track_since = time.monotonic()
            return True, 0.0
        return True, max(0.0, time.monotonic() - self._mosaic_track_since)

    def _reset_mosaic_last_frame_hold(self) -> None:
        self._mosaic_track_since = 0.0
        self._mosaic_holding_last = False
        self._mosaic_hold_until = 0.0
        self._mosaic_hold_urls = None

    def _hold_mosaic_slew_frame(self, pane: int, image: QImage | None = None) -> bool:
        ...
        if frame is None or frame.isNull():
            frame = self.live_images.peek(mosaic_live_still_camera())
        if frame is None or frame.isNull():
            frame = self._raw_preview_images.get(mosaic_live_still_camera()) or QImage()
        if frame is None or frame.isNull():
            return False
    ...
    def _mosaic_has_hold_frame(self, pane: int) -> bool:
        try:
            index = int(pane or 0)
        except (TypeError, ValueError):
            return False
        ...
        return live is not None and not live.isNull()

    def _mosaic_last_frame_ready(self, device_id: str = "", *, stacking: bool = False) -> bool:
        pane = self._mosaic_hold_pane(device_id)
        tracking, elapsed = self._mosaic_track_elapsed_s(device_id)
        timed_out = bool(self._mosaic_hold_until and time.monotonic() >= self._mosaic_hold_until)
        return mosaic_slew_frame_ready(
            has_frame=self._mosaic_has_hold_frame(pane),
            timed_out=timed_out,
            tracking=tracking,
            track_elapsed_s=elapsed,
            settle_s=_MOSAIC_TRACK_SETTLE_S,
            stacking=stacking,
        )

    def _finish_mosaic_last_frame_hold(self, device_id: str, tele_url: str, wide_url: str) -> None:
        pane = self._mosaic_hold_pane(device_id)
        self._hold_mosaic_slew_frame(pane)
        self._mosaic_holding_last = False
        self._mosaic_hold_until = 0.0
        self._mosaic_hold_urls = None
    ...
    def _refresh_mosaic_slew_placeholder(self, image: QImage) -> None:
        ...
        if pane < 1:
            return
        live = self._live_mosaic.get(self._selected_device_id) or {}
        tracking, elapsed = self._mosaic_track_elapsed_s()
        stacking = str(live.get("phase") or "").strip().lower() == "stacking" or self._mosaic_holding_last
        if mosaic_should_refresh_slew_placeholder(
            stacking_preview=False,
            frozen=self.mosaic_frames.frozen(pane),
            may_copy=self._mosaic_may_copy_live(pane),
            has_live_frame=True,
            tracking=tracking,
            track_elapsed_s=elapsed,
            settle_s=_MOSAIC_TRACK_SETTLE_S,
            ...
            self._hold_mosaic_slew_frame(pane, image)

    def _maybe_finish_mosaic_last_frame_hold(self, device_id: str = "") -> None:
        if not self._mosaic_holding_last:
            return
        owner = str(device_id or self._selected_device_id or "")
        pending = self._mosaic_hold_urls
        if not pending:
            self._reset_mosaic_last_frame_hold()
            return
        pane = self._mosaic_hold_pane(owner)
        if mosaic_should_hold_last_live_frame(pane):
            self._hold_mosaic_slew_frame(pane)
            if not self._mosaic_last_frame_ready(owner, stacking=True):
                return
        tele_url, wide_url = pending
        self._finish_mosaic_last_frame_hold(owner, tele_url, wide_url)
    ...
    def _publish_mosaic_pane_url(self, index: int, *, notify: bool = True) -> bool:
        ...
        left one pane blank on SKY. A failed GOTO must not invent a URL.
        """
        try:
            pane = int(index or 0)
        except (TypeError, ValueError):
            return False
        stored = self.mosaic_frames.peek(pane) if pane >= 1 else QImage()
        live = self._live_mosaic.get(self._selected_device_id) or {}
        phase = str(live.get("phase") or "")
        try:
            current = int(live.get("current_index") or 0)
        except (TypeError, ValueError):
            current = 0
        if not mosaic_should_publish_pane_url(
            pane=pane,
            has_still=not stored.isNull(),
            goto_failed=phase == "failed",
            failed_pane=current if phase == "failed" else 0,
        ):
            return False
        url = live_frame_data_url(stored)
        key = str(pane)
    ...
    def _snapshot_mosaic_pane(self, index: int, *, from_live: bool | None = None) -> None:
        if index < 1:
            return
        live_state = self._live_mosaic.get(self._selected_device_id) or {}
        phase = str(live_state.get("phase") or "")
        goto_failed = phase == "failed"
        try:
            failed_pane = int(live_state.get("current_index") or 0) if goto_failed else 0
        except (TypeError, ValueError):
            failed_pane = 0
        stored = self.mosaic_frames.peek(index)
    ...
    def _keep_mosaic_pane_raw(self, index: int, *, force: bool = False) -> None:
        ...
        if existing is not None and not existing.isNull() and not force:
            return
        if not self._mosaic_may_copy_live(index):
            return
    ...
    def _hold_live_mosaic_sheet(self, device_id: str, index: int) -> bool:
        """Snapshot the stacked pane and keep the contact sheet through user-stop.

        During GOTO the current index is the next pane. Freezing leftover live
        there is the slew-into-N+1 / trees-in-pane-3 bug.
        """
        if device_id == self._selected_device_id:
            live = self._live_mosaic.get(device_id) or {}
            pane = mosaic_finished_pane(
                self._mosaic_stream_pane,
                mosaic_result_pane(index, str(live.get("phase") or "")),
            )
            if pane >= 1:
                self._snapshot_mosaic_pane(pane)
        _active, columns, rows, _current, images = self.mosaic_frames.snapshot()
        keep = live_mosaic_keep_sheet(False, True, columns, rows, images)
        if keep:
    ...
    def _sync_mosaic_preview(self, device_id: str = "", previous: dict[str, Any] | None = None) -> None:
        ...
        live = self._live_mosaic.get(owner) or {}
        try:
            live_index = int(live.get("current_index") or 0)
        except (TypeError, ValueError):
            live_index = 0
        if live_index >= 1:
            index = live_index
        ...
        keep_live = mosaic_preview_keep_live_sheet(
            ...
            worker_running=bool(live.get("worker_running")),
            mosaic_active=active,
            capturing=self._session_is_capturing(owner),
    ...
    def mosaicPreview(self) -> dict[str, Any]:
        try:
            ...
            live = self._live_mosaic_for()
            current_session = live.get("current") if live else None
            session, _members = (None, [])
            if current_session is None:
                session, _members = self._mosaic_context()
                if session is not None:
    ...
    def skyMosaicPaneUrls(self) -> dict[str, str]:
        active, columns, rows, _current, _images = self.mosaic_frames.snapshot()
        showing = self._mosaic_keep_live_sheet() or mosaic_result_holds_sheet(
            preview_result=self._preview_result,
            active=active,
            columns=columns,
            rows=rows,
    ...
    def _sky_overlay_payload(
        ...
        center = target
        if (
            (center is None or center.ra_hours is None or center.dec_degrees is None)
            and view_ra == view_ra
        ):
        ...
        if center is not None and center.ra_hours is not None and center.dec_degrees is not None and grid_ok:
            try:
                ...
                self._sky_mosaic_center = Target(
                    ...
                    ra_hours=float(center.ra_hours),
    ...
    def _tick(self) -> None:
        ...
        if self._photo_capture_clock_active() or (
            self._telemetry_tick % 15 == 0 and any(worker.connected for worker in self._workers.values())
        ):
            # Refresh derived stale markers, and tick record/timelapse clocks.
            self._notify_devices()
    ...
    def startPreview(self, device_id: str) -> None:
        ...
        if preview_needs_rtsp_restart(telemetry):
            self.add_log(
                "info",
                "Stacking JPEG leftover — restarting live RTSP cameras",
                device_id,
            )
        ...
        def after_photo(ok: bool, result: Any) -> None:
            ...
            def open_after_mode(mode_ok: bool = True, mode_result: Any = None) -> None:
                if token != self._preview_token:
                    return
                if restore_dso:
                    if mode_ok:
                        ...
                        self.add_log(
                            "warning",
                            f"Live cameras restarted, but DSO mode did not restore: {mode_result}",
                            device_id,
                        )
                if device.model in (DeviceModel.DWARF_3, DeviceModel.DWARF_MINI):
                    after_cameras(True, result)
                    return
                self._open_legacy_preview_cameras(worker, device, token, after_cameras)

            if restore_dso:
                self.add_log("info", "Restoring DSO mode after restarting live cameras", device_id)
                worker.send("astro_mode", callback=open_after_mode)
                return
            open_after_mode()

        persisted_mode = device.control_settings.shooting_mode
        preserve_mode = preview_should_preserve_shooting_mode(
            self._device_telemetry.get(device_id),
            persisted_mode,
        )
        restore_dso = preview_should_restore_dso(
            self._device_telemetry.get(device_id),
            persisted_mode,
        )
        ...
        def after_live(ok: bool, result: Any) -> None:
            ...
            if preserve_mode:
                self.add_log("info", "Opening live cameras without changing shooting mode", device_id)
                self._open_preview_without_mode_switch(worker, device, token, after_cameras)
                return
    ...
    def _open_legacy_preview_cameras(
        ...
        device: Device,
        token: int,
        after_cameras: Callable[[bool, Any], None],
    ) -> None:
    ...
    def _open_preview_without_mode_switch(
        self,
        worker: Any,
        device: Device,
        token: int,
        ...
        """Open RTSP cameras while leaving DSO / solar / tracking alone."""
    ...
    def _mosaic_keep_live_sheet(self, device_id: str = "") -> bool:
        owner = str(device_id or self._selected_device_id or "")
        live = self._live_mosaic.get(owner) or {}
        session, members = self._mosaic_context(owner)
        return mosaic_preview_keep_live_sheet(
            live_phase=str(live.get("phase") or ""),
            live_stopping=bool(live.get("stopping")),
            worker_running=bool(live.get("worker_running")),
            mosaic_active=self._mosaic_is_active(session, members),
            capturing=self._session_is_capturing(owner),
            capture_continues=self._mosaic_capture_continues(owner),
        )
    ...
    def _clear_preview_result(self) -> None:
        ...
        drop_sheet = mosaic_result_should_drop_sheet(
            preview_result=self._preview_result,
            held=self._mosaic_result_held,
            frames_active=held_active,
            columns=held_cols,
            rows=held_rows,
            mosaic_running=self._mosaic_keep_live_sheet(),
        )
        had_result = bool(self._preview_result or self._preview_result_title or self._preview_result_detail)
    ...
    def _sync_preview_for_capture(
        ...
        if self._device_is_stopping(device_id):
            return
        stacking = self._preview_stacking(device_id)
        if self._mosaic_holding_last:
            if stacking:
                self._maybe_finish_mosaic_last_frame_hold(device_id)
                return
            self._reset_mosaic_last_frame_hold()
        if stacking and not self._preview_active:
        ...
        force_mosaic = mosaic_stack_preview_should_retarget(
            live_pane,
            self._mosaic_stream_pane,
            stacking,
            mode_changed,
        ...
        if mode_changed or urls_changed or force_mosaic:
            ...
            if not stacking and was_stacking:
                ...
                return
            if stacking and force_mosaic and was_stacking and live_pane != self._mosaic_stream_pane:
                if self._mosaic_stream_pane >= 1:
                    self._snapshot_mosaic_pane(self._mosaic_stream_pane)
            ...
            if stacking and not was_stacking:
                ...
                if mosaic_should_hold_last_live_frame(pane):
                    self._hold_mosaic_slew_frame(pane)
                    if not self._mosaic_last_frame_ready(device_id, stacking=True):
                        self._mosaic_holding_last = True
                        self._mosaic_hold_until = time.monotonic() + _MOSAIC_LAST_FRAME_WAIT_S
                        self._mosaic_hold_urls = (tele_url, wide_url)
                        if self._mosaic_keep_live_sheet(device_id):
                            self.mosaicPreviewChanged.emit()
                        return
                    self.add_log(
                    ...
                    self._finish_mosaic_last_frame_hold(device_id, tele_url, wide_url)
                    return
            self._set_preview_stack_mode(stacking)
            if stacking and force_mosaic and live_pane >= 1 and live_pane != self._mosaic_stream_pane:
                self.add_log(
                    "info",
                    f"Mosaic pane {live_pane} — reconnecting stacking preview",
                    device_id,
                )
            self._retarget_preview_streams(tele_url, wide_url)
            if stacking:
    ...
    def _detach_mosaic_stacking_preview(self) -> None:
        ...
        GoLive after a pane closes tele and puts live JPEG on the same HTTP
        URL. The tele player then ingested the wide landscape into pane 3.
        """
        self._reset_mosaic_last_frame_hold()
        self._set_preview_stack_mode(False)
        self._pending_retarget = None
    ...
    def _on_camera_frame(self, camera: str, image) -> None:
        ...
        if camera == mosaic_live_still_camera() and not raw.isNull():
            self._refresh_mosaic_slew_placeholder(raw)
            if self._mosaic_holding_last:
                self._maybe_finish_mosaic_last_frame_hold()
        if first_frame:
            if camera == "wide":
    ...
    def _abort_active_session(self, device_id: str, reason: str) -> bool:
        ...
        if not session_id:
            if running_live:
                self.add_log("warning", f"Stopping mosaic ({reason})", device_id)
                return True
            return False
    ...
    def _command_stack_target(self, device_id: str) -> Target:
        ...
        if center is not None and str(center.name or "").strip():
            return center
        telemetry = self._device_telemetry.get(device_id) or {}
        name = str(
            telemetry.get("capture_target")
            or telemetry.get("tracking_target")
            or getattr(self._sky_target, "name", None)
            or "Manual stack"
        ).strip() or "Manual stack"
        if is_mosaic_pane_name(name):
            parent = mosaic_group_title(name)
            if parent:
                name = parent
        coords = self._target_coords(self._sky_target)
        if coords is None:
            coords = self._target_coords(self.trackedSkyTarget)
        if coords is not None:
            return Target(
                name=name,
                kind=TargetKind.EQUATORIAL,
    ...
    def _persist_command_sessions(self, sessions: list[Session]) -> list[Session]:
        saved = [self._save_session(item, notify=False) for item in sessions]
        if saved:
            self._emit_sessions_changed()
        return saved

    def _adopt_command_session(self, device_id: str, session: Session, *, step: str = "Starting") -> Session:
        started = session.actual_started_at or datetime.now(timezone.utc).isoformat()
        ...
        previous = self._active_sessions.get(device_id)
        if previous and previous != session.id:
            self._command_panel_sessions.discard(previous)
        self._active_sessions[device_id] = session.id
        self._command_panel_sessions.add(session.id)
        self._session_timing.setdefault(
            session.id,
            {
        ...
        self._session_capture_base.setdefault(session.id, self._telemetry_frame_count(device_id))
        self._session_capture_peak.setdefault(session.id, 0)
        self._emit_sessions_changed()
        return session

    def _start_command_stack_session(self, device_id: str, camera: str) -> Session | None:
        if self._active_sessions.get(device_id):
            return None
        device = self._device_by_id(device_id)
        try:
            choice = Camera(str(camera or "").strip().lower())
        except ValueError:
            choice = device.camera if device is not None else Camera.TELE
        settings = replace(self._device_stack_camera_settings(device_id), camera=choice)
        start = self._store_session_time(self._now_local(device), device)
        saved = self._persist_command_sessions(
            sessions_for_command_stack(
                target=self._command_stack_target(device_id),
                device_id=device_id,
        ...
        if not saved:
            return None
        return self._adopt_command_session(device_id, saved[0], step="Starting stack")

    def _sync_command_mosaic_members(self, device_id: str, live: dict[str, Any]) -> None:
        members = [item for item in (live.get("members") or []) if isinstance(item, Session)]
        if not members or not any(self.store.sessions.get(item.id) for item in members):
            return
        current = live.get("current")
        for member in members:
            ...
            self.store.sessions.save(
                replace(
                    ...
                    actual_started_at=member.actual_started_at or stored.actual_started_at,
                    actual_ended_at=member.actual_ended_at,
                )
            )
            if member.status == SessionStatus.RUNNING or stored.status == SessionStatus.RUNNING:
                self._command_panel_sessions.add(member.id)
        if isinstance(current, Session) and self.store.sessions.get(current.id):
            if self._active_sessions.get(device_id) != current.id:
                self._adopt_command_session(
                    device_id, current, step=str(current.current_step or "Stacking")
                )
                return
            self._apply_session_step(current, str(current.current_step or "Stacking"))
            return
        self._emit_sessions_changed()

    def _cancel_command_mosaic_sessions(
        self, live: dict[str, Any] | None, *, outcome: str = "Cancelled"
    ) -> None:
        if not live:
            return
        members = [item for item in (live.get("members") or []) if isinstance(item, Session)]
        ...
        for member in members:
            stored = self.store.sessions.get(member.id)
            self._command_panel_sessions.discard(member.id)
            self._session_timing.pop(member.id, None)
            self._session_capture_base.pop(member.id, None)
            self._session_capture_peak.pop(member.id, None)
            self._stop_requested.discard(member.id)
            if stored is None or stored.status in {SessionStatus.DONE, SessionStatus.ERROR}:
                continue
            self.store.sessions.save(
                replace(
                    stored,
                    ...
                    outcome=outcome,
                    actual_ended_at=stored.actual_ended_at or ended,
                )
            )
            changed = True
        if members:
            device_id = members[0].device_id
            if self._active_sessions.get(device_id) in {item.id for item in members}:
                self._active_sessions.pop(device_id, None)
        if changed:
            self._emit_sessions_changed()

    def _finish_command_mosaic_sessions(
        self,
        device_id: str,
        live: dict[str, Any],
        ok: bool,
        ...
        ended = datetime.now(timezone.utc).isoformat()
        try:
            last = max(0, int(live.get("current_index") or 0))
        except (TypeError, ValueError):
            last = 0
        device = next((item for item in self._devices if item.id == device_id), None)
        records = history_records_for_live_mosaic(
            members,
            dict(live.get("captured") or {}),
            ok=ok,
            stopped=stopped,
            current_index=last,
            ended_at=ended,
        ...
        for index, member in enumerate(members, start=1):
            ...
            if stored is None:
                continue
            if index < last or (index == last and ok):
                outcome = "Completed"
                status = SessionStatus.DONE
            elif index == last:
                outcome = "Stopped by user" if stopped else str(result or "Failed")
                status = SessionStatus.ERROR
            else:
                outcome = "Cancelled"
                status = SessionStatus.ERROR
            self.store.sessions.save(
                replace(
                    ...
                    outcome=outcome,
                    actual_started_at=member.actual_started_at or stored.actual_started_at,
                    actual_ended_at=member.actual_ended_at or (ended if index <= last else stored.actual_ended_at),
                )
            )
        self._active_sessions.pop(device_id, None)
        if records:
    ...
    def _start_live_mosaic_stack(self, device_id: str, camera: str) -> bool:
        ...
        if existing and existing.get("phase"):
            self._cancel_command_mosaic_sessions(existing)
        ...
        pa = self._mosaic_pa(target)
        panes = self._cached_sky_preview_panes(columns, rows, overlap, pa, fov_h, fov_v)
        if panes and target is not None and target.ra_hours is not None and target.dec_degrees is not None:
            if not mosaic_panes_match_center(
                panes, float(target.ra_hours), float(target.dec_degrees), fov_h, fov_v
            ):
                panes = []
        if not panes:
            if target is None or target.ra_hours is None or target.dec_degrees is None:
        ...
        if len(panes) < 2:
            return False
        points = {
            (round(float(pane["ra_hours"]), 5), round(float(pane["dec_degrees"]), 5))
            for pane in panes
        }
        if len(points) < 2:
            self._toast(
                "Mosaic panes share one pointing",
                "error",
                "The sky grid collapsed to the centre. Refresh the SKY mosaic and try again.",
            )
            return False
        center_ra = float(target.ra_hours)
        center_dec = float(target.dec_degrees)
        started_at = datetime.now(timezone.utc).isoformat()
        scheduled_start = self._store_session_time(self._now_local(device), device)
        camera_settings = replace(
            self._device_stack_camera_settings(device_id),
            camera=mosaic_stack_camera(camera),
        )
        members = self._persist_command_sessions(
            sessions_for_command_stack(
                target=target,
                device_id=device_id,
        ...
        if len(members) < 2:
            self._cancel_command_mosaic_sessions({"members": members})
            return False
        payload: list[dict[str, Any]] = []
        for session in members:
            index = mosaic_pane_index(session.name) or (len(payload) + 1)
            payload.append({
                "index": index,
                "ra_hours": float(session.target.ra_hours or 0),
                "dec_degrees": float(session.target.dec_degrees or 0),
                "name": session.name,
                "row": int(session.mosaic.row or 0),
                "column": int(session.mosaic.column or 0),
                "session_id": session.id,
            })
            self._remember_named_sky_coords(session.name, session.target.ra_hours, session.target.dec_degrees)
        group_id = str(members[0].mosaic.group_id or f"live-mosaic-{device_id[:8]}-{uuid4().hex[:8]}")
        self._adopt_command_session(device_id, members[0], step="Starting mosaic")
        self._live_mosaic[device_id] = {
            "group": group_id,
            "columns": columns,
            "rows": rows,
            "total": len(members),
        ...
        self._mosaic_result_dismissed = False
        self._clear_mosaic_preview()
        self.mosaic_frames.set_layout(columns, rows, 1, True, group_id)
        self.mosaicPreviewChanged.emit()
        worker.busy = True
        worker.availabilityChanged.emit()
        grid = f"{columns}×{rows}"
        coord_text = " · ".join(
            f"{item['index']} {item['ra_hours']:.3f}h {item['dec_degrees']:+.2f}°"
            for item in payload
        )
        self.add_log(
            "notice",
            f"Stack mosaic {grid} · {target.name} · centre {center_ra:.3f}h {center_dec:+.2f}° · {len(members)} panes · {coord_text}",
            device_id,
        )
        self._toast(f"Stacking {grid} mosaic", "info", f"{len(members)} panes around {target.name}")
    ...
    def _on_live_mosaic_progress(self, device_id: str, pane: int, total: int, step: str) -> None:
        ...
        if total:
            live["total"] = max(int(live.get("total") or 0), int(total))
        ...
        if previous and index != previous:
            ...
            if previous <= len(members):
                ended = datetime.now(timezone.utc).isoformat()
                prev = members[previous - 1]
                captured = dict(live.get("captured") or {})
                captured[previous] = stacked_capture_count(self._device_telemetry.get(device_id) or {})
                ...
                members[previous - 1] = replace(
                    ...
                    actual_started_at=prev.actual_started_at or live.get("started_at") or prev.scheduled_start,
                )
        phase = mosaic_progress_phase(text)
        if phase:
            live["phase"] = phase
            if phase == "complete":
        ...
        if 1 <= index <= len(members):
            current = replace(
                members[index - 1],
                status=SessionStatus.RUNNING,
                current_step=text or "Stacking",
                actual_started_at=members[index - 1].actual_started_at
                or datetime.now(timezone.utc).isoformat(),
            )
            members[index - 1] = current
            live["current"] = current
            live["label"] = current.name
            self._remember_named_sky_coords(
                current.name, current.target.ra_hours, current.target.dec_degrees
            )
            if device_id == self._selected_device_id:
                self._set_preview_coords(current.name, self._target_coords(current.target))
        live["current_index"] = index
        live["members"] = members
        ...
        if device_id == self._selected_device_id:
            ...
            self._sync_preview_for_capture(device_id, telemetry, telemetry)
        self._notify_devices()
    ...
    def _finish_live_mosaic(self, device_id: str, ok: bool, result: Any) -> None:
        ...
        if ok:
            self.add_log("success", f"Mosaic complete · {total} panes", device_id)
            self._toast("Mosaic complete", "success", f"Stacked {total} panes")
            if device_id == self._selected_device_id:
                self._freeze_stacking_preview_result(
                    device_id,
                    title="MOSAIC COMPLETE",
                    detail=f"Finished {total} panes.",
                    target=label,
                    camera=self._preview_camera_name(device_id),
                    since=self._stack_result_since_for(device_id),
                )
        elif stopped:
            self.add_log("warning", f"Mosaic stopped · pane {index}/{total or index}", device_id)
            self._toast("Mosaic stopped", "warning", f"Stopped on pane {index}" if index else "")
            if self._telemetry_capturing(device_id):
                self.add_log(
                    "warning",
                    "Mosaic stopped; capture still running — relying on queued stop_astro",
                    device_id,
                )
        else:
            if mosaic_goto_failed_result(result):
                live_state = live or {}
                try:
                    fail_index = int(live_state.get("current_index") or index or 0)
                except (TypeError, ValueError):
                    fail_index = index
                panes = list(live_state.get("panes") or [])
                pane = None
                for item in panes:
                    if not isinstance(item, dict):
                        continue
                    try:
                        if int(item.get("index") or 0) == fail_index:
                            pane = item
                            break
                    except (TypeError, ValueError):
                        continue
                ra = pane.get("ra_hours") if isinstance(pane, dict) else None
                dec = pane.get("dec_degrees") if isinstance(pane, dict) else None
                self.add_log("error", f"Mosaic GOTO failed · {result}", device_id)
                coord = ""
                try:
                    if ra is not None and dec is not None:
                        coord = f"RA {float(ra):.4f}h Dec {float(dec):+.3f}°"
                except (TypeError, ValueError):
                    coord = ""
                self._toast(
                    "Mosaic GOTO failed",
                    "error",
                    str(result or coord or "GOTO failed; this pane was not stacked"),
                )
            else:
                self.add_log("error", f"Mosaic failed · {result}", device_id)
    ...
    def deviceAction(self, device_id: str, operation: str) -> None:
        ...
        def done(ok: bool, result: Any) -> None:
            ...
            if ok:
                ...
                if operation == "stack":
                    ...
                    if (
                        session_id
                        and session_id in self._command_panel_sessions
                        and not self._live_mosaic_running(device_id)
                    ):
                        self._finalize_session(session_id, False, result)
                        self._command_panel_sessions.discard(session_id)
        ...
        if operation == "track":
            ...
            if columns > 1 or rows > 1:
                ...
                return
            leftover = self._live_mosaic.pop(device_id, None)
            if leftover:
                self._cancel_command_mosaic_sessions(leftover)
                self.store.clear_live_mosaic(device_id)
                self._clear_mosaic_preview()
        ...
        if operation == "stop_astro":
            ...
            if self._live_mosaic_running(device_id, live):
                live["stopping"] = True
                self._hold_live_mosaic_sheet(device_id, int(live.get("current_index") or 0))
                self.mosaicPreviewChanged.emit()
    ...
    def reorderPlanned(self, session_id: str, before_session_id: str, night: str = "") -> None:
        ...
        if not session:
            return
        sessions = self.store.sessions.all()
        moving = planned_reorder_block(sessions, session)
        if moving is None:
            self._toast("Stop the running session before moving it", "warning")
            return
        if not moving:
        ...
        if before and before.device_id != session.device_id:
            before = None
            before_session_id = ""
        before_session_id = reorder_anchor_id(sessions, before, moving_ids)
        if before_session_id:
            before = self.store.sessions.get(before_session_id)
        else:
            before = None
        device = self._device_by_id(session.device_id)
        tz = self._zone_for(device)
        ...
        if target_day != old_night:
            delta = timedelta(days=(target_day - old_night).days)
            moving = [
                replace(
                    item,
                    scheduled_start=store_local_iso(
                        parse_in_zone(item.scheduled_start, tz) + delta,
                        tz,
                    ),
                )
                for item in moving
        ...
        ordered = insert_reorder_block(queue, moving, before_session_id)
        if not ordered:
            return
        starts = [
            parse_in_zone(item.scheduled_start, tz)
            for item in queue
    ...
    def generateStellariumMosaic(
        ...
        try:
            target_hint = parse_sky_web_target(payload)
        except (TypeError, ValueError, json.JSONDecodeError):
            target_hint = None
        pa = self._resolved_mosaic_pa(target_hint, position_angle)
        fov_h, fov_v, _camera = self._mosaic_fov()
        ...
        def work() -> dict[str, Any]:
            target = self._resolve_stellarium_target(payload)
            pa_now = self._resolved_mosaic_pa(target, position_angle)
            extra = sky_web_template_notes(payload)
            panes = cached
            if abs(float(pa_now) - float(pa)) > 1e-4:
                panes = []
            if (
                panes
                and target.ra_hours is not None
                and target.dec_degrees is not None
                and not mosaic_panes_match_center(
                    panes, float(target.ra_hours), float(target.dec_degrees), fov_h, fov_v
                )
            ):
                panes = []
    ...
    def _handle_async_result(self, operation: str, result: tuple[bool, Any]) -> None:
        ...
        if operation == "stellariumRcPing":
            ...
            return
        if operation == "skyCatalogResolve":
            payload = value if isinstance(value, dict) else {}
            device_id = str(payload.get("device_id") or "")
            name = str(payload.get("name") or "").strip()
            resolve_key = f"{device_id}:{name.lower()}"
            if self._catalog_resolve_key == resolve_key:
                self._catalog_resolve_key = ""
            if ok and isinstance(payload.get("hit"), dict) and not is_mosaic_pane_name(name):
                self._catalog_resolve_cooldown.pop(resolve_key, None)
                self._apply_catalog_hit(device_id, payload["hit"], announce=True, query_name=name)
                return
            self._catalog_resolve_cooldown[resolve_key] = time.monotonic() + 45.0
            commanded = self._commanded_sky_coords(name)
            if commanded is not None:
                self._set_preview_coords(name, commanded)
                return
            if name and not is_mosaic_pane_name(name):
                self.add_log(
                    "warning",
                    f"No catalog match for {name}. Firmware tracking packets include the name only, not RA/Dec.",
                    device_id,
                )
            return
        if operation == "skyLockResolve":
            self._sky_lock_inflight = False
            payload = value if isinstance(value, dict) else {}
            name = str(payload.get("name") or "").strip()
            commanded = self._commanded_sky_coords(name)
            coords = commanded or (self._target_coords(payload) if ok else None)
            if commanded is not None:
                self._emit_sky_lock(name, commanded)
                return
            if coords is None or payload.get("need_pointing") or is_mosaic_pane_name(name):
                label = name or "the tracked target"
                self._toast(
                    "Could not lock that target on the sky map",
                    "warning",
                    f"No catalog match for {label}",
                )
                return
            self._apply_catalog_hit(str(self._selected_device_id or ""), payload, announce=True, query_name=name)
            self._emit_sky_lock(name, coords, list(payload.get("aliases") or []))
            return
    ...
    def _start_session(self, worker: TelescopeProcess, session: Session) -> None:
        ...
        if resuming and not session.mosaic.imported_plan and session.mosaic.panes > 1:
            try:
                pane = int((self._device_telemetry.get(session.device_id) or {}).get("mosaic_index") or 0)
            except (TypeError, ValueError):
                pane = 0
            if pane >= 1:
    ...
    def _telemetry_frame_count(self, device_id: str) -> int:
        return stacked_capture_count(self._device_telemetry.get(device_id) or {})

    def _track_session_capture(self, device_id: str, telemetry: dict[str, Any]) -> None:
        session_id = self._active_sessions.get(device_id)
        if not session_id:
            return
        observed = stacked_capture_count(telemetry)
    ...
    def _device_stack_camera_settings(self, device_id: str) -> CameraSettings:
        device = next((item for item in self._devices if item.id == device_id), None)
        telemetry = self._device_telemetry.get(device_id) or {}
        camera = device.camera if device is not None else Camera.TELE
        defaults = device.capture_defaults if device is not None else CaptureDefaults()
        settings = camera_settings_from_capture(defaults, camera=camera)
        exposure = exposure_seconds_from_text(
            telemetry.get("exposure_text") or telemetry.get("astro_exposure_text")
        )
        if exposure:
            settings = replace(settings, exposure_seconds=float(exposure))
        try:
            count = int(float(telemetry.get("stack_count") or telemetry.get("capture_total") or 0))
        except (TypeError, ValueError):
            count = 0
        if count >= 1:
            settings = replace(settings, frame_count=count)
        return settings

    def _track_manual_capture(
        self,
        device_id: str,
        previous: dict[str, Any],
        current: dict[str, Any],
        ...
        owner = capture_history_owner(
            session_active=bool(self._active_sessions.get(device_id)),
            mosaic_running=mosaic_running,
            session_finalizing=device_id in self._pending_session_finish,
        )
        was = bool(previous.get("capture_active") or previous.get("capture_state") == "running")
        now = bool(current.get("capture_active") or current.get("capture_state") == "running")
        ...
        if session_id and session_id in self._command_panel_sessions:
            ...
            if now:
                if not was:
                    self._session_capture_base.setdefault(session_id, stacked)
                self._session_capture_peak[session_id] = max(
                    self._session_capture_peak.get(session_id, 0), stacked
                )
                return
            if not was:
                return
            stopped = session_id in self._stop_requested
            self._finalize_session(session_id, not stopped, "Stopped by user" if stopped else "")
            self._command_panel_sessions.discard(session_id)
            return
        if owner != "manual":
            if now:
        ...
        if now:
            if not was:
                self._manual_capture_started[device_id] = datetime.now(timezone.utc).isoformat()
                self._manual_capture_peak[device_id] = stacked
            else:
                self._manual_capture_peak[device_id] = max(
                    self._manual_capture_peak.get(device_id, 0), stacked
                )
            return
        if not was:
            return
        peak = max(self._manual_capture_peak.pop(device_id, 0), stacked)
        started = self._manual_capture_started.pop(device_id, None)
        if not started:
            return
        self._write_manual_stack_history(device_id, current, started, peak)

    def _write_manual_stack_history(
        self,
        device_id: str,
        ...
        if capture_history_owner(
            session_active=bool(self._active_sessions.get(device_id)),
            mosaic_running=bool(self._live_mosaic.get(device_id)),
            session_finalizing=device_id in self._pending_session_finish,
        ) != "manual":
            return
        ...
        record = history_record_for_manual_stack(
            device_id=device_id,
            target_name=name,
            camera=self._device_stack_camera_settings(device_id),
            started_at=started,
            ended_at=ended,
            captured_frame_count=captured,
            outcome=outcome,
    ...
    def _write_live_mosaic_history(
        self,
        device_id: str,
        live: dict[str, Any],
        ok: bool,
        ...
        records = history_records_for_live_mosaic(
            ...
            ok=ok,
            stopped=stopped,
            current_index=int(live.get("current_index") or 0),
            ended_at=ended,
            result=result,
            hardware=device.hardware if device else None,
        )
    ...
    def nightTimelineIso(self, day: str, minutes: int, device_id: str = "") -> str:
        try:
            ...
            return ""
        value = start + timedelta(minutes=max(0, min(1439, int(minutes))))
